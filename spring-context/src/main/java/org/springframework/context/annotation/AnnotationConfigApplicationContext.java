...
public class AnnotationConfigApplicationContext extends GenericApplicationContext implements AnnotationConfigRegistry {
	...
	/**
	 * 初始化一个bean读取器和扫描器
	 * 默认构造方法，如果直接调用这个默认的构造方法，需要在稍后通过调用其register方法去注册配置类（javaconfig）
	 * 并调用refresh方法刷新容器，触发容器对注解bean的载入、解析和注册过程
	...
	public AnnotationConfigApplicationContext() {
		 ...
		 * 这里会初始化一些spring默认的BeanDefinition放到DefaultListableBeanFactory维护的map中（没错！就是那6个bd）
		 */
		this.reader = new AnnotatedBeanDefinitionReader(this);
		/**
		 * 创建一个扫描器
		 * 能够扫描一个类，包 并且转化成BeanDefinition
	...
	/**
	 * 这个构造方法须要传入一个被javaConfig注解了的配置类
	 * 然后会把这个被注解了javaConfig的类通过注解读取器读取和解析
